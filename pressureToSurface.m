...
solveFunct(iniGuessX,x,N)
% - - - - - - - - - - - - - - - - - - - 
% Use the internal Matlab "Newton's Method
% - - - - - - - - - - - - - - - - - - - 
options = optimset('TolFun',1e-8,'jacobian','off','display','iter');
...
for j=-Nn:Nn
    ...
    sigma(jIndx,:) = coeff.*exp(1i*j*x).*cosh(j*(eta+h))/pi;
end
...
function dF = jacobian(X,x,N)
    ...
    for k = -Nn:Nn
        ...
        for j=-Nn:Nn
            ...
            int = @(x)exp(-1i*j*x)*(c-sqrt(c^2-2.*(P-rho*g*h)));
            coeff = integral(int,0,2*pi,'ArrayValued',true);
            sigma(jIndx,:) =(1/pi).*coeff.*exp(1i*j*x).*sinh(j*(eta+h)).*(j);
        end
