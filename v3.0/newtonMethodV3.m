function returnVal = newtonMethodV3(iniGuessEtaHat,iniGuessC, a, h, g, rho, N, L, x)
    ...
    function uHat = hat(u,x,N)
            dx = x(2)-x(1);
            L = x(end)-x(1);
            intTerm = cos(N*x*pi/L).*u;      % create a matrix mesh of the Fourier integrand
    ...
    function dF = jacobian(X,x,N)
        ...
        for n = 0:Nn
            if n==0
                ...
                for k = 0:Nn
                    dFdEta = cos(k*pi*x/L).*(n*(c^2-2*g*eta).*cosh(n*(eta+h))-g*sinh(n*(eta+h)));
